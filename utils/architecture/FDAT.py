...
"""
Fast Dual Aggregation Transformer (FDAT) architecture.
Adapted from https://github.com/stinkybread/fdat/blob/main/fdat.py
"""
...
from utils.architecture.fdat_utils import store_hyperparameters, DropPath
...
class FDATNet(nn.Module):
    def __init__(self, state_dict, scale=None, num_in_ch=None, num_out_ch=None, num_feat=None, num_block=None, **kwargs):
        ...
        if missing_keys:
            print(f"Warning: Missing keys in FDAT model: {missing_keys[:5]}...")  # Show first 5
        if unexpected_keys:
            print(f"Warning: Unexpected keys in state_dict: {unexpected_keys[:5]}...")  # Show first 5
    
    def _detect_scale(self, state_dict):
        """Detect scale factor from state dict"""
        # Try to find upsampler metadata
        for key in state_dict.keys():
            if "upsampler.MetaUpsample" in key:
                meta = state_dict[key]
                if len(meta) > 2:
                    return int(meta[2])
        
        # Try to infer from upsampler structure
        # Look for transpose convolution patterns
        for key in state_dict.keys():
            if "upsampler.0.weight" in key:
                weight = state_dict[key]
                if len(weight.shape) == 4 and weight.shape[2] == weight.shape[3]:
                    kernel_size = weight.shape[2]
                    if kernel_size == 4:  # Common for 2x upscale
                        return 2
                    elif kernel_size == 3:  # Could be 3x
                        return 3
        
        # Default scale
        return 4
    
    def _detect_in_channels(self, state_dict):
    ...
    def _detect_out_channels(self, state_dict):
        """Detect output channels from state dict"""
        # Look for final upsampler layer - try multiple possible final layers
        final_layers = []
        for key in state_dict.keys():
            if "upsampler" in key and "weight" in key:
                ...
                if len(parts) >= 2 and parts[1].isdigit():
                    ...
                    if len(weight.shape) == 4:  # Conv2d weight
                        final_layers.append((layer_num, weight.shape[0]))
        
        if final_layers:
            # Get the last layer's output channels
            final_layers.sort()
            return final_layers[-1][1]
        
        return 3
    
    def _detect_num_feat(self, state_dict):
        """Detect number of features from state dict"""
        for key in state_dict.keys():
            if "conv_first.weight" in key:
    ...
    def _detect_upsampler_type(self, state_dict):
        ...
        # Check for MetaUpsample buffer first
        for key in state_dict.keys():
            if "upsampler.MetaUpsample" in key:
                meta = state_dict[key]
                if len(meta) > 1:
                    upsampler_idx = int(meta[1])
                    upsampler_types = list(SampleMods3.__args__)
                    if 0 <= upsampler_idx < len(upsampler_types):
                        return upsampler_types[upsampler_idx]
        
        # Fallback detection based on layer structure
        has_transpose = any("ConvTranspose" in str(type(v)) for v in state_dict.values() if hasattr(v, 'dtype'))
        if has_transpose:
            return "transpose+conv"
        
        return "transpose+conv"  # Default
    
    def _detect_mid_dim(self, state_dict):
        """Detect mid_dim from state dict"""
        # Try to find from MetaUpsample
        for key in state_dict.keys():
            if "upsampler.MetaUpsample" in key:
                meta = state_dict[key]
                if len(meta) > 5:
