...
"""
Dual Aggregation Transformer (DAT) architecture support.
Adapted from https://github.com/lovindata/pillow-dat/blob/main/PIL_DAT/_dat_arch.py
"""
...
# Wrapper class for compatibility with ESRGAN loading system
class DATNet(nn.Module):
    def __init__(self, state_dict, scale=None, num_in_ch=None, num_out_ch=None, num_feat=None, num_block=None, **kwargs):
        super(DATNet, self).__init__()
        
        # Extract model parameters from state_dict
        self.scale = self._detect_scale(state_dict) if scale is None else scale
        self.in_nc = self._detect_in_nc(state_dict) if num_in_ch is None else num_in_ch
        self.out_nc = self._detect_out_nc(state_dict) if num_out_ch is None else num_out_ch
        ...
        # Detect additional DAT-specific parameters
        ...
        if missing_keys:
            print(f"Warning: Missing keys in DAT model: {missing_keys[:5]}...")  # Show first 5
        if unexpected_keys:
            print(f"Warning: Unexpected keys in DAT model: {unexpected_keys[:5]}...")  # Show first 5

    def _detect_scale(self, state_dict):
        # Try to detect scale from upsample layers
        for key in state_dict.keys():
            if 'upsample' in key and 'weight' in key:
                weight = state_dict[key]
                if len(weight.shape) == 4:
                    out_ch = weight.shape[0]
                    # For pixel shuffle: out_channels = scale^2 * output_channels
                    if out_ch == 12:  # 2^2 * 3
                        return 2
                    elif out_ch == 48:  # 4^2 * 3  
                        return 4
                    elif out_ch == 27:  # 3^2 * 3
                        return 3
        return 4  # Default scale

    def _detect_in_nc(self, state_dict):
        # Check conv_first layer
        if 'conv_first.weight' in state_dict:
            return state_dict['conv_first.weight'].shape[1]
        return 3  # Default RGB

    def _detect_out_nc(self, state_dict):
        # Check conv_last layer or final output layer
        for key in state_dict.keys():
            if 'conv_last.weight' in key:
                return state_dict[key].shape[0]
            elif key.endswith('.weight') and 'conv' in key:
                weight = state_dict[key]
                if len(weight.shape) == 4 and weight.shape[0] in [1, 3, 4]:
                    return weight.shape[0]
        return 3  # Default RGB

    def _detect_num_feat(self, state_dict):
        # Check conv_first layer output channels
        if 'conv_first.weight' in state_dict:
            return state_dict['conv_first.weight'].shape[0]
        return 64  # Default

    def _detect_depth(self, state_dict):
        # Count number of residual groups and blocks per group
        max_group = 0
        blocks_per_group = {}
        ...
        for key in state_dict.keys():
            if 'layers.' in key and 'blocks.' in key:
                try:
                    ...
                    max_group = max(max_group, layer_idx)
                    if layer_idx not in blocks_per_group:
                        blocks_per_group[layer_idx] = set()
                    blocks_per_group[layer_idx].add(block_idx)
                except (ValueError, IndexError):
                    pass
        
        if blocks_per_group:
            # Convert to depth list
            depth = []
            for i in range(max_group + 1):
                if i in blocks_per_group:
                    depth.append(len(blocks_per_group[i]))
                else:
                    depth.append(2)  # Default
            return depth
        
        return [2, 2, 2, 2]  # Default

    def _detect_expansion_factor(self, state_dict):
        # Try to detect expansion factor from FFN layers
        embed_dim = self._detect_num_feat(state_dict)
        
        for key in state_dict.keys():
            if 'ffn.fc1.weight' in key:
                fc1_out_dim = state_dict[key].shape[0]
                expansion_factor = fc1_out_dim / embed_dim
                return expansion_factor
        
        return 4.0  # Default

    def _detect_num_heads(self, state_dict):
        # Try to detect number of heads from attention layers
        depth = self._detect_depth(state_dict)
        
        # Look for temperature parameter in channel attention to get actual num_heads
        for key in state_dict.keys():
            if 'attn.temperature' in key:
                temp_shape = state_dict[key].shape
                if len(temp_shape) >= 1:
                    num_heads_detected = temp_shape[0]
                    return [num_heads_detected] * len(depth)
        
        # Look for position bias layers in spatial attention
        for key in state_dict.keys():
            if 'pos.pos3.2.weight' in key:
                pos_out_dim = state_dict[key].shape[0]
                # This should match num_heads // 2 for spatial attention
                spatial_heads = pos_out_dim * 2
                return [spatial_heads] * len(depth)
        
        # Fallback: try to estimate from embedding dimension
        embed_dim = self._detect_num_feat(state_dict)
        if embed_dim <= 64:
            return [2] * len(depth)  # Small model
        elif embed_dim <= 120:
        ...
        for key in state_dict.keys():
            if 'layers.' in key:
                try:
                    ...
                    max_group = max(max_group, layer_num)
                except:
                    pass
        return max_group + 1 if max_group > 0 else 4  # Default
